...
class Video extends CI_Controller {
  ...
  public function open_video()
    ...
    public function appointment_list1()
    {
        ...
        foreach($data as $row)
        {
            ...
            <tr>
             <td>'.$row['doctor_id'].'</td>
             <td>'.$row['appointment_date'].'</td>
             <td> <a href="https://callncure.com/index.php/videocall/patient?id='.$row['id'].'" id="doc_id" class="btn btn-success btn-xs" data-id="'.$row['id'].'"> Click Here</a> </td>               
            </tr>
          ';
        }
        echo $new;
    ...
    public function appointments_list()
    {
        header("Access-Control-Allow-Origin:*");
        $user_id=$this->input->post('user_id');
        $where=array('doctor_id' => $user_id,'appointment_date' => date('Y-m-d'));
        $data=$this->AppointmentModel->get_data('appointment','id',$where);
        // $new='';
        ...
        {
            // echo json_encode($data);
            $new=array('found'=>'yes','dataes'=>$data);
            echo json_encode($new);
        }else{
             $new=array('found'=>'no');
            echo json_encode($new);
    ...
    public function answer_call()
    {
        ...
        if(count($data) > 0)
        {
            $patient=$this->db->where('id',$data[0]['appoinment_id'])->order_by('id','DESC')->get('appointment')->result_array();
            $new=array('valid' => 'yes',
            'room_id' => $data[0]['video_id'],
    ...
    public function change_appointment_status()
    {
        ...
        $where=array('id' => $id);
        $data=array('status' => 2);
        $data=$this->AppointmentModel->update_data('appointment',$where,$data);

        $where=array('appoinment_id' => $id);
        $data=array('seen_id' => 1);
        $data=$this->AppointmentModel->update_data('tbl_model_video_id',$where,$data);
        // if(!empty($data))
        // {
        //     $new=array(
        //     'valid' => 'yes',
